...
// Cập nhật lại Interface Props của TaskCard để nhận thêm onPress
interface TaskProps {
  group: string;
  title: string;
  time: string;
...
const TaskCard = ({
  ...
  return (
    // Đổi thẻ View ngoài cùng thành TouchableOpacity
    <TouchableOpacity style={styles.card} onPress={onPress} activeOpacity={0.8}>
      ...
      <View style={styles.rightColumn}>
        <View
          style={[
            styles.iconBox,
            { backgroundColor: isDone ? "#FFEBFA" : "#F0EEFF" },
          ]}
        >
          <IconComponent
            size={18}
            color={isDone ? "#FF4FC3" : "#5F33E1"}
          />
        </View>
        ...
        <View
          style={[
            styles.statusBadge,
            { backgroundColor: isDone ? "#F0EEFF" : "#FFF0E6" },
          ]}
        >
          <Text
            style={[
              styles.statusText,
              { color: isDone ? "#5F33E1" : "#FF8A00" },
            ]}
          >
            {status}
          </Text>
      ...
      <View style={styles.taskListArea}>
        <FlatList
          ...
          contentContainerStyle={{
            ...
            paddingBottom: 120, // Tăng nhẹ để không bị che bởi BottomNavigation
          }}
          renderItem={({ item }) => (
            // Bỏ TouchableOpacity bọc ngoài, chuyển onPress vào làm Props của TaskCard
            <TaskCard
              group={item.group}
              title={item.title}
              time={item.time}
